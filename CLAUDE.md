...
- audit残項目（2026-07-04 棚卸し。詳細な状況・優先順位は docs/audit-2026-07.md の
  ...
  - B1（受信マージの空配列early-return）：着手前に方針をerisaと相談してから実装する。
    論点＝「isEmptyガード（GAS誤空からの保護）」と「tombstone適用による削除伝播」の両立方法。
    syncFromCloud の心臓部6ブロックを触るため、複数端末での削除伝播シナリオでの動作確認が
    必要。単独セッション推奨。B2（血液検査・体重・薬のtombstone未対応）は着手時に併せて検討。
