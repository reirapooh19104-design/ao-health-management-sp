...
### 19. tombstone欠落 → 経路④の書き戻しで削除項目が復活する（next_visit「テスト1」事件・2026-07-07）
- **症状**：削除した端末では消えるが、GASシート本体（B列）に該当エントリが残存し、
  他端末が受信して表示し続ける。リロードでも消えない**安定した不整合**。
  （PCは表示されず・GAS本体に残存・スマホは表示継続、の3点が同時に成立）
- **確定原因**：削除時に **tombstone が打たれていなかった**（該当ID
  `note_1783342568761_f8dznp` がどの端末の deleted_ids.next_visit にも無かった）。
  next_visit の受信マージは union 構造で「削除は filterByTombstone だけが行う」設計
  （落とし穴B1対応の意図）のため、tombstone が無いと消せない。さらに経路④の書き戻しが
  「ローカルにあってGASに無い項目」を**本体ごとGASへ再注入**し続け、復活が固定化する。
- **tombstone が打たれない既知の2経路**：
  - (A) **文字列レガシー項目（idなし）の削除**：`deleteNextVisitItem` は `if (id)` 条件付きで
    addDeletedId を呼ぶため（index.html 4000-4002行付近）、id を持たない項目は tombstone なし。
  - (B) **`saveConsultation()` の全クリア**：診察保存時に `setNextVisitItems([])` で全消しするが
    tombstone は1件も打たない（consultation_summary の `setConsultationSummary([])` も同様）。
    → この経路で消した項目は他端末に削除が伝わらず、経路④で復活しうる。
- **復旧手順（2026-07-07 実施・収束）**：削除対象IDをコンソールで deleted_ids.next_visit に
  追加 → 本体配列からも該当IDを除去 → pushAllToCloud → 全端末リロードで収束。
- 補足：tombstone の push は addDeletedId 内の単キー型 pushToCloud（fire-and-forget・
  `_ts_`なし・到達確認なし）。この不達も、GAS の deleted_ids に tombstone が載らない一因になりうる。
...
## import個別push抑止→pushAll集約（2026-07-07 対応済み・①クローズ）
- インポート中は個別pushを完全抑止し、完了後に pushAllToCloud を1回だけ発射する方式
  （コミット 2acb8dc）。busy嵐（落とし穴16の306件）を構造的に解消。
- 実装：`_importing` フラグを新設し、**合流点である pushPairToCloud / pushToCloud の冒頭**に
  `if (_importing) return;` ガードを置く（save()内ではなくpush関数側に置くことで、
  chk_ デバウンスタイマー経路・通常経路の両方を1箇所で塞ぐ）。加えて save() の chk_ 分岐で
  `_importing` 中は **pending登録・setTimeout自体をスキップ**（フラグ解除後にタイマーから
  発射される漏れを封鎖）。localStorage保存・`_ts_`刻印は従来どおり抑止しない。
- importData は `_importing = true` で全setterを囲み、完了後に解除 → renderAll →
  `if (getGasUrl() && getSyncToken()) await pushAllToCloud();`（**URL/合言葉設定時のみ**。
  落とし穴17の「URL未設定インポート」では送信ゼロを維持）。**try/finally でフラグ復帰を保証**
  （立ちっぱなしだと以後の全pushが無音で消える最悪の退行になるため必須）。
- pushAll は deleted_ids も収集対象に含むため、mergeDeletedIds 分の削除伝播もこの1回で送られる。
- 実機検証済み（2026-07-07）：インポート中の個別POST **0本** → 完了後 pushAll **1本**、
  `[混雑]` **306件→0件**、成功342件/失敗0件、インポート後の通常編集でpush正常発射
  （フラグが false に復帰していることを確認）。

## 次セッション候補（2026-07-07 更新）

### 実害あり・優先度高め（実際に事象を観測した対策）
- **③ tombstone欠落経路の穴埋め**（落とし穴19）：`saveConsultation()` の全クリア時と
  レガシー項目（idなし）削除時に tombstone を打つ。あわせて tombstone push
  （addDeletedId 内の単キー型 pushToCloud・fire-and-forget）の到達性向上（bulk化/到達確認）も
  検討する。**本物のあお記録でも同種の復活が起こり得るため優先度高め**。
...
### 対応済み（記録）
- **① import の個別push抑止 → pushAll集約**：【対応済み 2026-07-07／コミット 2acb8dc】
  詳細は上記「import個別push抑止→pushAll集約」セクション参照。busy 306件→0件を実機確認。
