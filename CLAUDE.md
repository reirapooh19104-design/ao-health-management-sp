...
## 次セッション候補（2026-07-05 更新）

### 実害あり・優先度高め（今日実際に事象を観測した対策）
...
- **② 本体+`_ts_` の1POST統合**：pushToCloud が本体と `_ts_` を独立2フェッチ
  （fire-and-forget）で送るため、片方だけのネットワーク失敗を検知できない
  （今日の recipes 本体不達が実例）。1POSTに統合して不整合を解消する。
...
- **残課題（LockServiceでも防げない・次セッション候補②）**：pushToCloud は本体と `_ts_` を
  **独立したfire-and-forgetの2フェッチ**（`await`なし・res.ok未確認）で送るため、
  **本体フェッチ1本だけがネットワーク層で失敗**すると「_tsだけ着地・本体は旧値のまま」に
  なる（ロックは同時書き込みの衝突を防ぐだけで、片方のフェッチ不達は防げない）。
  実例：2026-07-05、recipes 全量インポート直後の個別pushで本体だけ不達→シートが
  recipes=[1]・_ts_recipes=新 のズレ状態になった。本体+_tsの1POST統合でこの不整合も解消する。
