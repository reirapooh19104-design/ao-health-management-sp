...
## 未解決の申し送り（2026-07-04 発見／2026-07-05 原因確定・修正方針は未決定）

### 編集・追加がGASに書き戻されず他端末に伝わらない（原因確定：doPostの半書き込みレース）
- **【原因確定 2026-07-05 実機調査】save() の2POST同時発射 × GAS doPost のレース条件**：
...
- **修正方針は未決定・次セッションで方針議論→実装**。候補（単独or併用）：
  1. GAS側に LockService を導入（doPost の読み書きをロックで直列化）
  2. クライアント側で本体＋`_ts_` を1POSTに統合（レースの発生源自体を無くす）
- **当面の運用（修正まで）**：save() 経由の単発保存はGASに残らないことがある。
  確実にGASへ上げたいときは「全データGAS同期」（pushAllToCloud・1POSTのbulk送信）を使う。
  - **注意**：pushAllToCloud（bulk）は「その端末のローカルデータでクラウドを上書き」する。
    現在は7/4 10:41以降の単発編集がGASに届いておらず、スマホでの編集はスマホの
    localStorageにしか存在しない可能性があるため、**修正完了までPCからのbulk送信は
    原則行わない**（スマホ側の未同期分を巻き込む復旧計画を立ててから実施する）。

### データ復旧（修正後に実施・要方針議論）
- シートには7/4 10:41以降の単発編集（save経由）が欠けている。
- 各端末のローカルには保存されているためデータ損失はないが、
  端末ごとに「自分だけが持つ編集」があり得る状態。
- 復旧手順の候補：各端末でエクスポート → 内容確認・マージ →
  レース修正のデプロイ後に一括同期。順序設計を修正方針議論と同時に行う。
