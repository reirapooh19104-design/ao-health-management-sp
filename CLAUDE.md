...
- **修正方針の候補（単独or併用）と状況**：
  1. GAS側に LockService を導入（doPost/doGet の読み書きをロックで直列化）
     → 【対応済み 2026-07-05／コミット 1ff4ee9・GASデプロイ v7・実機3回連続検証済み】
  2. クライアント側で本体＋`_ts_` を1POSTに統合（レースの発生源自体を無くす）→ **未対応**
- **LockServiceでも防げない残課題（候補2の意義）**：pushToCloud は本体と `_ts_` を
  ...
  **本体フェッチ1本だけがネットワーク層で失敗**すると「_tsだけ着地・本体は旧値のまま」に
  ...
  recipes=[1]・_ts_recipes=新 のズレ状態になった。候補2（本体+_tsの1POST統合）は
  この不整合も同時に解消する。
