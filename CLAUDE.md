...
- **固定情報の編集**：名前・犬種・避妊・食事・アレルギー・既往・ホメオパシーは `PRINT_CONSTANTS`（`const DENSITY` の直後付近）にまとめて定義。ここを変えれば印刷資料に自動反映される。
...
- **データ取得元**：体重＝`getBodyWeights()`、サプリ＝`getSupple()`、薬＝`getMedList()`、レシピ＝`getRecipes()`＋`_currentRecipeId`、血液検査＝`getBloodTests()`、体重＝直近10件、あおの記録＝`getAoRecords()`、次回来院時＝`getNextVisitItems()`。
