...
## カレンダーの表示ルール（renderCalendar）（2026-07-15 整理）
- **1つの意味に1つの見せ方を割り当てる**方針。カレンダーに表示を足すときは、
  空いているチャンネルを使うこと（既存チャンネルへの相乗りは落とし穴20を招く）。
  - **セルの背景色** ＝ ウルソの飲む/休む（`.drink` / `.rest`）。服用終了・休止中は色なし。
  - **赤枠＋薄い赤の塗り** ＝ 特記事項あり（`.special-note`）。
  - **数字を囲むピンクの丸** ＝ 今日（`.cal-d.today .cal-d-num`）。
  - **下段のドット** ＝ 咳（紫）・食欲不振（オレンジ）・元気度低下（赤）。
  - **右上のハート** ＝ フィラリア投与日（`.cal-filaria-mark`・絶対配置）。
- 今日の印は `.cal-d.today` に対し**数字を包む span（`.cal-d-num`）を丸く塗る**方式
  （2026-07-15／コミット 4c6add1）。セルの背景・枠と取り合わないため、
  ウルソの色とも特記事項の赤枠とも共存でき、**ウルソ服用終了でも今日の印が出る**。
- `.cal-d.special-note` の background に `!important` は**不要**（付けないこと）。
  `.drink`/`.rest` と同じ詳細度で、ソース順で後ろにあるため従来どおり勝つ。
  かつて付いていたのは今日の背景塗りに勝つためで、その相手はもう存在しない。
- 凡例（`.cal-legend`）に「今日」の項目は**意図的に載せていない**（丸は説明不要と判断）。
- ウルソは2026-07-15 時点で**服用終了**（数値が上がらない限り再開予定なし）。
  そのため `.drink` / `.rest` の背景色は現在どの日にも出ていないが、再開時に備えて
  判定ロジック（`ursoStatus()`）ごと残してある。今日の丸は再開しても背景色の上に乗るだけ。
...
## 既知の落とし穴（2026-07-15 追記）

### 20. 表示チャンネルへの相乗りは、親の条件が消えると道連れで消える（対策済み）
- カレンダーの「今日」の印を**セルの背景色に相乗り**させていた（`today-drink`＝ピンク塗り／
  `today-rest`＝濃いグレー塗り）。背景色はウルソの飲む/休むを表すチャンネルなので、
  **ウルソが服用終了になると背景色ごと今日の印も消えた**。2026-07-15 に実際に発生し、
  「カレンダーで今日が分からない」状態になっていた（`ursoStatus()` が null を返すと
  `today-*` クラスが一切付かない構造）。
- 同じ相乗りが**特記事項との奪い合い**も生んでいた。`.special-note` が
  `background: #FFE8E8 !important` で背景を強制上書きするため、今日＋特記事項の日は
  今日の塗りが消え、`border: 2px solid #B71C1C`（濃い赤枠）という応急処置で
  かろうじて区別していた。**1つのチャンネル（背景色）に2つの意味が乗っていたのが根因**。
- 【対策済み 2026-07-15／コミット 4c6add1】今日に独立したチャンネル（数字を囲む丸）を与え、
  `today-drink` / `today-rest` / 応急処置の濃い赤枠の3行を削除。`!important` も除去。
  詳細は「カレンダーの表示ルール（renderCalendar）」セクション参照。
- 教訓：**「Aが表示されている時だけ意味を持つ見せ方」でBを表現しない**。
  Aが消える日（服用終了・機能停止・データなし）にBも道連れで消える。
  表示を足すときは、既存チャンネルへの相乗りではなく空きチャンネルを使うか、
  無ければ新しく作る。CSSの `outline` や、要素の内側（今回の数字を包む span）は
  `background` / `border` と衝突しない独立チャンネルとして使いやすい。
- 教訓2：`!important` は「別の意味とチャンネルを奪い合っている」サインのことがある。
  消す前に、なぜ必要になったのかを辿ると設計の重なりが見つかる。
