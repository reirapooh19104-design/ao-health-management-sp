...
- **次セッションで最初に確かめること（この順で）**：
  1. 【最重要】**PC側の編集は本当にGAS(スプレッドシート)に届いているか**を直接確認する。
     今日確認したのは「PC内での反映」のみ。PCで印付き編集→dataタブに反映されるか見る。
     「PCは届く/スマホは届かない」なら端末差、「PCも届かない」ならアプリ全体の書き戻し問題で
     切り分けが変わる。
  2. スマホの2つの exec それぞれの request body の **token の値を実際に読み**、同一か・
     正しい値かを比較（今日は token フィールドの存在は確認したが、値の一致までは未確認）。
  3. **ok と unauthorized が混在する理由**（対称なはずの2つのPOSTで認証結果が分かれる原因）を特定。
  - 補足：pushToCloud は fire-and-forget で失敗を握りつぶすため（落とし穴4・9）、
    ログは [push失敗]／[認証エラー] の有無も併せて見る。
- **繊細案件**：合言葉（SYNC_TOKEN）・GAS認証にからむため、原因確定まで**修正着手しない**。
