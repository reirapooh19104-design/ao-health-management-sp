...
- 削除の同期は tombstone（`deleted_ids`）で8カテゴリ実装済み：
  ...
  push back（GASへの書き戻し）を持つ4キー（next_visit/consultation_summary/
  consultation_history/ao_records）は2026-06-19にtombstone適用漏れを修正済み（history.md参照）。
