...
### deploy ステップだけが「Deployment failed, try again later.」で失敗する時
- **症状**：push 後、Actions の pages build and deployment で **build は成功するが
  deploy ステップだけが失敗**し、ログに `Deployment failed, try again later.` と出る。
  GitHub Pages 側の一時的な事象で、コードやリポジトリの問題ではない。繰り返し起きる既知現象。
- **対処の順序**：
  1. **まず数分待って、GitHub 側の自動リトライを確認する。** 自動リトライで同じコミットの
     deploy が緑になることがある（2026-07-04 に実際に発生）。緑になれば、それ以上何もしなくてよい。
  2. 自動リトライで緑にならない場合のみ、**空コミットで再トリガー**する：
     `git commit --allow-empty -m "chore: trigger pages redeploy"` → push。
  - ※ 自動リトライと空コミットが入れ違うと、空コミットが冗長になることがある（実害はない）。
    そのため順序は「まず自動リトライを待つ → ダメなら空コミット」が無駄がない。
- **確認**：最新の実行が緑になり、公開版URL（`?v=<ハッシュ>`）が正常に開ければ問題なし。
  コード無変更の空コミットならアプリ挙動には影響しない。
- **運用実績**：過去に f9e1939・1b89572・251d01d・bfe9153 等でこのパターンが繰り返し発生している。
